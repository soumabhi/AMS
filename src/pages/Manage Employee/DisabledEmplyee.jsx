...
const ActiveEmployees = () => {
  ...
  const [employeeToEnable, setEmployeeToEnable] = useState(null);
...
const fetchEmployees = async (page = 1, limit = 10) => {
  ...
  try {
    ...
    if (response.data) {
     const activeEmployees = response.data.employees.filter(emp => emp.status === "inactive");
...
const disableEmployee = async (id) => {
  ...
  try {
    ...
    showToast("Employee enabled successfully");
    // Refresh the employee list
    fetchEmployees(currentPage, pageSize);
  } catch (error) {
    console.error("Error enabling employee:", error);
    showToast(
      "Failed to enable employee: " +
...
const exportToExcel = () => {
  try {
    const exportData = employees.map(emp => ({
      ...
      const filename = `Disable_Employees_${moment().format('DD-MM-YYYY')}.xlsx`;
 ...
 const handleConfirmUpload = async () => {
  ...
  const handleEnableClick = (employee) => {
        ...
        <div className="flex-1">
          <div className="bg-gradient-to-r from-white to-gray-50 border border-gray-200 rounded-2xl shadow-xl p-6">
            ...
            <div className="flex items-center justify-between mb-6 text-left">
              <h1 className="text-2xl font-bold text-gray-900">Disable  Employees</h1>
      ...
      <tr key={employee._id} className="hover:bg-gradient-to-r hover:from-gray-50 hover:to-gray-100 transition-all duration-200">
        ...
        <td className="px-6 py-4 whitespace-nowrap flex gap-2">
          <button
            ...
            className="p-2 text-blue-400 hover:text-blue-600 hover:bg-gray-100 rounded-lg transition-colors"
            title="View"
          >
          ...
          <button
            ...
            className="p-2 text-green-400 hover:text-green-600 hover:bg-gray-100 rounded-lg transition-colors"
            title="Enable"
          >
            ...
            <div className="p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-xl font-bold text-gray-900">Enable Employee</h2>
                </div>
                <button
              ...
              <p className="text-gray-600 mb-6">
                Are you sure you want to enable {selectedEmployee.name}? This action cannot be undone.
              </p>
